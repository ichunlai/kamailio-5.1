...
static inline struct addr_info *new_addr_info(
    char          *name,
    enum si_flags gf)
{
    struct addr_info *al;
...
static inline void free_addr_info(
    struct addr_info *a)
{
    if (a)
    {
...
static inline void free_addr_info_lst(
    struct addr_info **lst)
{
    struct addr_info *a;
 ...
 * returns 0 on success, -1 on error */
static int new_addr_info2list(
    char *name, enum si_flags f,
    struct addr_info **l)
{
    struct addr_info *al;
...
static inline struct socket_info *new_sock_info(
    char *name,
 ...
 */
int socket2str(
    char *s, int *len, struct socket_info *si)
{
...
/* returns 0 if support for the protocol is not compiled or if proto is
   invalid */
struct socket_info **get_sock_info_list(
    unsigned short proto)
{
    switch (proto)
    {
...
inline static int si_hname_cmp(
    str *host, str *name, str *addr_str,
...
static struct socket_info *new_sock2list(
    char *name, struct name_lst *addr_l,
    unsigned short port,
    unsigned short proto,
    char *usename, unsigned short useport,
    enum si_flags flags,
 ...
 * return  new sock info on success, 0 on error */
static struct socket_info *new_sock2list_after(
    char               *name,
    struct name_lst    *addr_l,
    unsigned short     port,
    unsigned short     proto,
...
int add_listen_advertise_iface(
    char *name, struct name_lst *addr_l,
    unsigned short port, unsigned short proto,
    char *usename, unsigned short useport,
    enum si_flags flags)
...
int addattr_l(
    struct nlmsghdr *n, int maxlen, int type, const void *data,
    int alen)
{
 ...
 */
int add_interfaces_via_netlink(
    char *if_name, int family, unsigned short port,
    unsigned short proto,
...
static int fix_hostname(
    str *name, struct ip_addr *address, str *address_str,
 ...
 */
static int addr_info_to_si_lst(
    struct addr_info *ai_lst, unsigned short port,
 ...
 * return 0 on succes, -1 on error
 */
static int addr_info_to_si_lst_after(
...
static int fix_socket_list(
    struct socket_info **list, int *type_flags)
{
    struct socket_info *si;
    struct socket_info *new_si;
 ...
 */
int parse_protohostport(
    str *ins, sr_phostp_t *r)
{
 ...
 */
struct socket_info *lookup_local_socket(
    str *phostp)
{
    sr_phostp_t r;
