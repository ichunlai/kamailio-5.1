...
unsigned int set_modinit_delay(
    unsigned int v)
{
    unsigned int r;
    r             = modinit_delay;
...
void set_child_sip_rpc_mode(
    void)
{
    child_sip_rpc_mode |= CHILD_SIP_RPC;
}

void set_child_rpc_sip_mode(
    void)
{
 ...
 */
static sr31_cmd_export_t *sr_cmd_exports_convert(
    unsigned ver,
    void *src, void *mod)
{
    int               i, n;
...
static int register_module(
    unsigned ver, union module_exports_u *e,
...
static inline int version_control(
 ...
 */
static inline int sr_cmd_flags_match(
 ...
 */
sr31_cmd_export_t *find_mod_export_record(
    char *mod, char *name,
 ...
 * flags parameter is OR value of all flags that must match
 */
sr31_cmd_export_t *find_export_record(
    char *name,
    int param_no, int flags,
    unsigned *mod_if_ver)
{
...
cmd_function find_export(
    char *name, int param_no, int flags)
{
    sr31_cmd_export_t *cmd;
    unsigned          mver;
...
struct sr_module *find_module_by_name(
    char *mod)
{
    struct sr_module *t;
...
sr_module_t *get_loaded_modules(
    void)
{
    return modules;
}
 ...
 */
void *find_param_export(
    struct sr_module *mod, char *name,
    ...
    int rank)
{
    ...
    switch (rank)
    {
    case PROC_MAIN:     type = "PROC_MAIN";
        break;
    case PROC_TIMER:    type = "PROC_TIMER";
        break;
    case PROC_FIFO:     type = "PROC_FIFO";
        break;
    case PROC_TCP_MAIN: type = "PROC_TCP_MAIN";
        break;
    default:            type = "CHILD";
        break;
    }
...
static int init_mod_child(
    struct sr_module *m, int rank)
{
    if (m)
    {
...
action_u_t *fixup_get_param(
    void **cur_param, int cur_param_no,
 ...
 */
action_param_type *fixup_get_param_ptype(
    void **param)
{
    action_u_t *a;
 ...
 */
action_param_type fixup_get_param_type(
    void **param)
{
 ...
 */
int get_str_fparam(
    str *dst, struct sip_msg *msg, fparam_t *param)
{
    int_str    val;
    int        ret;
    avp_t      *avp;
 ...
 */
int get_int_fparam(
    int *dst, struct sip_msg *msg, fparam_t *param)
{
    int_str    val;
    int        ret;
    avp_t      *avp;
 ...
 */
int get_is_fparam(
    int *i_dst, str *s_dst, struct sip_msg *msg, fparam_t *param, unsigned int *flags)
{
    int_str    val;
    int        ret;
    avp_t      *avp;
 ...
 */
int get_regex_fparam(
    regex_t *dst, struct sip_msg *msg, fparam_t *param)
{
    switch (param->type)
    {
    case FPARAM_REGEX:
 ...
 */
free_fixup_function get_fixup_free(
    fixup_function f)
{
